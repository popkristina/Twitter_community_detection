...
json_file <- lapply(readLines("election-day-retreived.json"), fromJSON)
...
for(i in 1:nrow(to_export_df)){
  for(j in 1:ncol(to_export_df)){
    if(grepl("England",to_export_df[i,j]) || 
       ...
       grepl("Brasil",to_export_df[i,j]) ||
       grepl("Rio",to_export_df[i,j]) ||
       grepl("Turkey",to_export_df[i,j]) ||
       grepl("Istambul",to_export_df[i,j]) ||
       grepl("Syria",to_export_df[i,j]) ||
       grepl("Germany",to_export_df[i,j]) ||
       grepl("Berlin",to_export_df[i,j]) ||
       grepl("Frankfurt",to_export_df[i,j]) ||
       grepl("Prague",to_export_df[i,j]) ||
       grepl("Madrid",to_export_df[i,j]) ||
       grepl("Rome",to_export_df[i,j]) ||
       grepl("Mexico",to_export_df[i,j]) ||
       grepl("Chile",to_export_df[i,j]) ||
       grepl("Parag",to_export_df[i,j]) ||
       grepl("Argentina",to_export_df[i,j]) ||
       grepl("Greece",to_export_df[i,j]) ||
       grepl("Bangkok",to_export_df[i,j]) ||
       grepl("Thailand",to_export_df[i,j]) ||
       grepl("India",to_export_df[i,j]) ||
       grepl("Naples",to_export_df[i,j]) ||
       grepl("Jamaica",to_export_df[i,j]))
    to_export_df<-to_export_df[-i,]
  }
}
write.xlsx(to_export_df,"../Cleaned/election-day-cleaned.xlsx",row.names = FALSE)